...
=======
    ...
    private const float TextLocalY =  1f;
    ...
    /// <summary>
    /// Returns the UnityEngine.Color that matches the current TextColor setting.
    /// IMPORTANT: The TMP component's .color is always kept at Color.white so that
    /// rich-text &lt;color&gt; tags (used for role names) are not multiplied away.
    /// Black is therefore applied as a &lt;color&gt; wrapper in UpdateDisplay, not here.
    /// This method returns white for both Black and White settings for that reason.
    /// </summary>
    private static Color GetTextColor()
    {
        var settings = LocalSettingsTabSingleton<NotePadLocalSettings>.Instance;
        return settings.TextColor.Value switch
        {
            ...
            // White and Black both keep TMP component white;
            // black is injected via <color=#000000> wrapper in UpdateDisplay.
            _ => Color.white,
        };
>>>>>>> Stashed changes
    }
    ...
    /// <summary>
    /// Returns the hex color string to wrap plain text in, or null if no wrapper
    /// is needed (i.e. the effective color is already white).
    /// </summary>
    private static string? GetPlainTextColorTag()
    {
        ...
        return settings.TextColor.Value switch
        {
            NotepadTextColor.Black  => "#000000",
            NotepadTextColor.Red    => null, // handled by component color
            NotepadTextColor.Yellow => null,
            NotepadTextColor.Green  => null,
            NotepadTextColor.Cyan   => null,
            NotepadTextColor.Grey   => null,
            _                       => null, // White — no wrapper needed
        };
    }
    ...
    {
        var settings = LocalSettingsTabSingleton<NotePadLocalSettings>.Instance;
        return settings.WindowSkin.Value switch
        {
            NotepadWindowSkin.Black => "NotePadMod.Resources.notepad_window_black.png",
            _                       => "NotePadMod.Resources.notepad_window.png",
        };
    }
...
=======
        ...
        // Flush again on close so the game doesn't lurch when input is re-enabled.
        Input.ResetInputAxes();
>>>>>>> Stashed changes
    }
...
=======
        ...
        // Refresh TMP component color in case the setting was changed while open.
        if (_displayTmp != null)
...
=======
        ...
        // Apply role-name color tags first, then optionally wrap the whole string
        // in a plain-text color tag for Black (other colors are set on the TMP
        // component directly so they don't need a wrapper).
...
=======
            _displayTmp.fontSize           = 2.2f;
            // Keep component color at white so <color> rich-text tags are unaffected.
            _displayTmp.color              = GetTextColor();
            _displayTmp.enableWordWrapping = true;
            _displayTmp.overflowMode       = TextOverflowModes.Overflow;
            _displayTmp.enableAutoSizing   = false;
